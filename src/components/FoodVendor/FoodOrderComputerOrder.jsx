...
import { useSelector } from "react-redux"; // Assuming userId from store
import { getAllCartItems } from "@/firebaseDemo/demofirestore";
import { useCartItems, useChangeCartQuantity, useRemoveFromCart } from "@/hooksDemo/userMutation";

const CartItem = ({ name, desc, price, img,id ,quantity}) =>{ 
  console.log(id,"is wokring")
  ...
  <div className="flex items-center justify-between bg-white rounded-lg p-3 shadow-sm">
    ...
    <div className="flex flex-col items-end gap-2">
      ...
      <div className="flex items-center gap-1">
        <button onClick={()=>{
          changeQuantity({ productId:id, delta: -1 })
...
const FoodOrderComputerOrderCard = () => {
  ...
  const {userId} = useSelector((state) => state.user); // Get userId from store
