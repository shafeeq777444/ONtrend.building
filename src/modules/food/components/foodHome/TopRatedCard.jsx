...
const TopRatedCards = ({ vendor }) => {
  ...
  return (
    <div
      ...
      className="relative w-full max-w-lg mx-auto group overflow-hidden rounded-md shadow-sm h-65 2xl:h-85 cursor-pointer"
    >
      {/* Banner Image */}
      {vendor.bannerImage?.length > 0 && (
        <LazyImg
          ...
          alt="Food Item"
          className="aspect-4/3 object-cover rounded-md transition-transform duration-500 ease-out group-hover:scale-105"
        />
      )}

      {/* Busy Overlay */}
      {!vendor.isOnline && (
        <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-10 opacity-60 group-hover:opacity-100 transition-opacity duration-500 ease-out">
          <span className="text-white text-md font-semibold">{isArabic ? "مشغول" : "Busy"}</span>
        </div>
      )}
