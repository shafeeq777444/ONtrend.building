...
export function subscribeTopVendors(lat, lng, onData, onError) {
    ...
    const q = query(usersRef, where("role", "==", "Vendor"), where("isApproved", "==", true), where("isTop", "==", true));
    ...
    return onSnapshot(
        ...
        async (snapshot) => {
            try {
                ...
                // ✅ If no vendors found nearby
                if (enrichedVendors.length === 0) {
                    onData({
                        vendors: [],
                        message: "Ontrend isn’t delivering here yet. Stay tuned — Ontrend is expanding!",
                    });
                    return;
                }

                // ✅ Normal vendors response
                onData({
