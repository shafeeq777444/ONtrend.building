...
const TopPharmacies = ({ setBannerON }) => {
    ...
    if (isLoading || (topPharmacies.length === 0 && message === null)) {
        return (
            <div className="mt-4">
                <SkeletonTopPharmacies />
            </div>
        );
    }
    ...
    return (
        <div className="px-4 py-6 relative bg-white">
            <h2 className="text-xl font-bold mb-4 pl-4">{isArabic ? "أفضل الصيدليات" : "Top Pharmacies"}</h2>

            <button
                className={`swiper-button-prev-pharmacy hidden md:block ${
                    isArabic ? "left-10" : "right-14"
                } custom-nav absolute top-0  z-10 bg-white p-2 rounded-full shadow mt-4 hover:bg-gray-100 transition`}
            >
                <FiChevronLeft size={20} />
            </button>

            <button
                className={`swiper-button-next-pharmacy hidden md:block custom-nav absolute top-0 ${
